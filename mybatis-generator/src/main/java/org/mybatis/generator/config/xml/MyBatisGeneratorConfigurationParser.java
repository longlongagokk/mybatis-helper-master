...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseContext(Configuration configuration, Node node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            } else if ("javaQueryModelGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseJavaQueryModelGenerator(context, childNode);
            } else if ("javaRequestModelGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseJavaRequestModelGenerator(context, childNode);
            } else if ("javaViewModelGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseJavaViewModelGenerator(context, childNode);
            ...
            } else if ("javaConsoleControllerGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseJavaConsoleControllerGenerator(context, childNode);
    ...
    private void parseSqlMapGenerator(Context context, Node node) {
        ...
        String isMergeable = attributes.getProperty("isMergeable"); //$NON-NLS-1$

        sqlMapGeneratorConfiguration.setTargetPackage(targetPackage);
        sqlMapGeneratorConfiguration.setTargetProject(targetProject);
        sqlMapGeneratorConfiguration.setMergeable(Boolean.valueOf(isMergeable));
    ...
    private void parseTable(Context context, Node node) {
        ...
        String _baseObjectNameWithNoTb;
        int tbIndex = tableName.indexOf("tb_");
        if(tbIndex == 0){
            _baseObjectNameWithNoTb = underscoreToCamelCase(tableName.substring(2));
        }else{
            _baseObjectNameWithNoTb = underscoreToCamelCase(tableName);
        }
        if(!stringHasValue(domainObjectName)){
            domainObjectName = "Tb" + _baseObjectNameWithNoTb;
        }else if(domainObjectName.indexOf("Tb") == 0){
            _baseObjectNameWithNoTb = domainObjectName.substring(2);
        }
        String mapperAndServiceFileName = attributes.getProperty("mapperAndServiceFileName");
        if(!stringHasValue(mapperAndServiceFileName)){
            mapperAndServiceFileName = _baseObjectNameWithNoTb;
        }
        String queryModelObjectName = attributes.getProperty("queryModelObjectName");
        String requestModelObjectName = attributes.getProperty("requestModelObjectName");
        String viewModelObjectName = attributes.getProperty("viewModelObjectName");
        if(!stringHasValue(viewModelObjectName)){
            viewModelObjectName = "Tv"+_baseObjectNameWithNoTb;
        }
        ...
        if(stringHasValue(mapperAndServiceFileName)){
            tc.setMapperAndServiceFileName(mapperAndServiceFileName);
        }
        if(stringHasValue(queryModelObjectName)){
            tc.setQueryModelObjectName(queryModelObjectName);
        }
        if(stringHasValue(requestModelObjectName)){
            tc.setRequestModelObjectName(requestModelObjectName);
        }
        if(stringHasValue(viewModelObjectName)){
            tc.setViewModelObjectName(viewModelObjectName);
        }
    ...
    private void parseJavaQueryModelGenerator(Context context, Node node) {
        JavaQueryModelGeneratorConfiguration javaQueryModelGeneratorConfiguration = new JavaQueryModelGeneratorConfiguration();

        context.setJavaQueryModelGeneratorConfiguration(javaQueryModelGeneratorConfiguration);

        Properties attributes = parseAttributes(node);
        String targetPackage = attributes.getProperty("targetPackage"); //$NON-NLS-1$
        String targetProject = attributes.getProperty("targetProject"); //$NON-NLS-1$
        String rootClass = attributes.getProperty("rootClass"); //$NON-NLS-1$

        javaQueryModelGeneratorConfiguration.setTargetPackage(targetPackage);
        javaQueryModelGeneratorConfiguration.setTargetProject(targetProject);
        javaQueryModelGeneratorConfiguration.setRootClass(rootClass);
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseProperty(javaQueryModelGeneratorConfiguration, childNode);
            }
        }
    }
    private void parseJavaRequestModelGenerator(Context context, Node node) {
        JavaRequestModelGeneratorConfiguration configuration = new JavaRequestModelGeneratorConfiguration();

        context.setJavaRequestModelGeneratorConfiguration(configuration);
        ...
        configuration.setRootClass(rootClass);
    ...
    private void parseJavaViewModelGenerator(Context context, Node node) {
        JavaViewModelGeneratorConfiguration javaViewModelGeneratorConfiguration = new JavaViewModelGeneratorConfiguration();

        context.setJavaViewModelGeneratorConfiguration(javaViewModelGeneratorConfiguration);

        Properties attributes = parseAttributes(node);
        String targetPackage = attributes.getProperty("targetPackage"); //$NON-NLS-1$
        String targetProject = attributes.getProperty("targetProject"); //$NON-NLS-1$

        javaViewModelGeneratorConfiguration.setTargetPackage(targetPackage);
        javaViewModelGeneratorConfiguration.setTargetProject(targetProject);
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseProperty(javaViewModelGeneratorConfiguration, childNode);
            }
        }
    }
    ...
    private void parseJavaConsoleControllerGenerator(Context context, Node node) {
        JavaConsoleControllerGeneratorConfiguration javaConsoleControllerGeneratorConfiguration = new JavaConsoleControllerGeneratorConfiguration();

        context.setJavaConsoleControllerGeneratorConfiguration(javaConsoleControllerGeneratorConfiguration);
        ...
        String targetProject = attributes.getProperty("targetProject"); //$NON-NLS-1$
        String implementationPackage = attributes.getProperty("implementationPackage"); //$NON-NLS-1$
        String basePath = attributes.getProperty("basePath"); //$NON-NLS-1$

        javaConsoleControllerGeneratorConfiguration.setConfigurationType(type);
        javaConsoleControllerGeneratorConfiguration.setTargetPackage(targetPackage);
        javaConsoleControllerGeneratorConfiguration.setTargetProject(targetProject);
        javaConsoleControllerGeneratorConfiguration.setImplementationPackage(implementationPackage);
        javaConsoleControllerGeneratorConfiguration.setBasePath(basePath);
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseProperty(javaConsoleControllerGeneratorConfiguration, childNode);
            }
        }
    }
