...
//		log.info("------------------------"+JSONUtil.toJSONString(mapper.selectItemByPrimaryKey(IdWrapper.valueOf(6,TsOrderForm.Fields.deliveryId,TsOrderForm.Fields.memberId)))+"------------------------------");
		...
		SelectWrapper queryWrapper =
				new SelectWrapper<>(new StringConditionWrapper())
//						.select("payWayId pid")
//						.select("payState sd")
//						.select("userName    ")
//						.select("   dealStatus    ")
//						.select("   orderDate   ")
						.select0(SelectField.valueOf("(select count(0) from Tb_cms_news) counts",true))
//						.select0(
...
//				)
						...
						.where(x->x
								.e()
								.eq(TsOrderForm.Fields.amountPaid,1)
								.d().eq("e.orderNo","123")
								.eq("e.orderNo","456")
								.or(m->m.eq("e.id",889)
										.eq("e.payState",3)
										.e()
										.eq(TsOrderForm.Fields.memberId,345)
								)
								.e()
								.eq(TsOrderForm.Fields.address,2)
								.eq(TsOrderForm.Fields.deliveryId,3)
								.eq(TsOrderForm.Fields.deliveryId,4)
								.or(a->a.eq(TsOrderForm.Fields.deliveryId,5)
										.and(b->b.eq(TsOrderForm.Fields.orderNo,"abc")
...
//				)
				.set(TsOrderForm.Fields.amountPay,88.88)

				.where(x->
						x
						.eq("e.deliveryId",1)
						.eq("e.id",2)
						.e().eq(TsOrderForm.Fields.leaveMessage,"343")
...
//								)
		...
		log.info("------------------------"+ JSONUtil.toJSONString(
				mapper.selectOne(SelectWrapper.build()
						...
						.select1(TsOrderForm.Fields.leaveMessage)
						.where(x->x.e()
								.eq(TsOrderForm.Fields.memberId,3))
				)
		)+"------------------------------");
